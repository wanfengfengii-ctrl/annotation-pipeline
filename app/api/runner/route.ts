  ...
  try {
    ...
    if (b.action === 'supply-context') {
      ...
      return Response.json({
        ...
        repos:
          ...
          ).length >= Math.min(3, config.concurrency)
            ? []
            : sources(config, tasks),
        ...
        candidateBuffer: 2,
        unfinishedProjects: tasks.filter(
          (t) => t.projectSeries && !t.closed && !projectQuotaComplete(t),
        ).length,
    ...
    if (b.action === 'enqueue-auto') {
      ...
      if (
        ...
        return Response.json({ skipped: '现有项目题额未满，优先在原项目续题' });
      ...
      if (typeof b.prompt !== 'string' || questionIssues(b.prompt).length)
        throw Error('自动题目不符合当前标题、段落或表达要求');
      ...
      const inserted = await db()
        ...
        AND (SELECT COALESCE(group_concat(entry,'|'),'') FROM (SELECT id||':'||revision entry FROM tasks ORDER BY id))=?`)
        .bind(
          task.id,
          serializeTask(task),
          now,
    ...
    if (b.action === 'reserve-claude') {
      ...
      if (
        ...
        !r.continuationOf &&
        !wasSent(r) &&
        replacementCategories.includes(r.category) &&
        sentProjectCounts(item.task)[r.category] >= 10
      )
        return Response.json({
          allowed: false,
          reason: '本项目该题型实际发送已达十题',
        });
    ...
    if (b.action === 'finish') {
      ...
      if (r.projectRetry) {
        ...
        const recovery = b.projectRecovery || {
          version: projectRecoveryVersion,
          turnId: r.id,
          state: 'blocked',
          attempts: (r.projectRecovery?.attempts || 0) + 1,
          reason: String(b.error || '续题执行器中断，原记录保留'),
          retryAt: new Date(Date.now() + 5 * 60 * 1000).toISOString(),
        };
        const originalStatus = r.projectRetry.originalStatus;
        r.status = originalStatus as Turn['status'];
        r.stage = r.projectRetry.originalStage;
        if (
          !recovery ||
          recovery.version !== projectRecoveryVersion ||
          recovery.turnId !== r.id ||
          recovery.attempts !== (r.projectRecovery?.attempts || 0) + 1 ||
          !['planned', 'blocked'].includes(recovery.state)
        )
          throw Error('同项目续题回执无效');
        r.projectRecovery = recovery;
        if (recovery.state === 'planned') {
          const c = recovery.candidate,
            snapshot = recovery.sourceSnapshot;
          if (
            !recovery.idleVerified ||
            !snapshot?.verified ||
            !/^[a-f0-9]{64}$/.test(snapshot.manifestSha256 || '') ||
            !replacementCategories.includes(c?.category) ||
            !canAddTurn(item.task, c.category) ||
            questionIssues(c.prompt, { category: c.category }).length ||
            exactGoalDuplicate(item.task, c.prompt)
          )
            throw Error('同项目续题依据、题额或题面无效');
          assertPolicyAudit(recovery.audit, await candidateDigest(c), {
            requireQuestionStyle: true,
          });
          ...
          item.task.turns.push({
            ...
            autoFollowup: true,
            projectSource: { turnId: r.id, ...snapshot },
          });
          item.task.automationNotice =
            '上一题原记录已保留，在当前项目排队执行新的' + c.category;
        } else
          item.task.automationNotice =
            '当前项目继续保留，续题待处理：' + recovery.reason;
      ...
      if (r.stageRecovery?.retrying)
        r.stageRecovery = {
          ...
          retryAt: new Date(Date.now() + 2 * 60 * 1000).toISOString(),
        };
