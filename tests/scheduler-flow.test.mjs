...
const schema=a[a.indexOf('--output-schema')+1],out=a[a.indexOf('--output-last-message')+1],stage=['generate','policy','prepare','snapshot','score','delivery','next','project-next'].find(s=>schema.endsWith('.'+s+'.schema.json'));
const values={'project-next':{action:'complete',prompt:'无',reason:'fixture complete',category:'Feature 迭代',difficulty:'中等',baseComplete:true,projectEvidence:'fixture file'},policy:{simpleFeatures:[],difficultyEvidence:['scope evidence','context evidence','interaction evidence','breadth evidence'],assessedDifficulty:'中等',followupFix:false,followupReason:'首轮或非产物修复',allowed:true,matchedRuleIds:[],duplicateTaskIds:[],checkedGroups:['games','desktop','business','dashboard'],reason:'synthetic eligible task'},generate:{title:'__SCHEDULER_FLOW_AUTO__',prompt:'Synthetic new task',category:'0-1 代码生成',difficulty:'中等',stack:'fixture'},prepare:{prompt:'Synthetic prepared task',category:input.includes('项目连续出题规则')?'0-1 代码生成':'代码测试',difficulty:'中等',stack:'fixture',acceptance:['synthetic']},snapshot:{environmentLevel:'无外部依赖',dependencies:[],startup:'fixture',verification:'fixture',ready:true,head:'a'.repeat(40),remote:'https://github.com/fixture/repo.git',notes:['synthetic']},next:{action:'complete',prompt:'无',reason:'fixture complete'},score:{when:Array(5).fill('fixture step'),behavior:Array(5).fill('fixture behavior'),impact:Array(5).fill('fixture impact'),expected:Array(5).fill('fixture expected'),evidenceRefs:Array(5).fill((input.match(/本轮轨迹文件：([^\\n]+)/)||[])[1]+':1'),processFindings:'fixture',artifactFindings:'fixture',scores:[3,3,3,3,3],descriptions:['a','b','c','d','e'],other:'无'},delivery:{passed:true,checks:['synthetic'],summary:'synthetic'}};log({event:stage});fs.writeFileSync(out,JSON.stringify(values[stage]));console.log(JSON.stringify({type:'thread.started',thread_id:'fixture'}));});`;
