...
export function questionRoot(task, turn) {
  ...
  if (!turn.continuationOf) return turn.id;
  const previous = task.turns?.find((r) => r.id === turn.continuationOf);
  if (!previous) throw Error('继续原题缺少前序轮次');
  const visited = new Set([turn.id]);
  let current = previous;
  while (current.continuationOf && !current.questionRootId) {
    if (visited.has(current.id)) throw Error('原题引用存在循环');
    visited.add(current.id);
    current = task.turns.find((r) => r.id === current.continuationOf);
