...
import { scoreGroundingInstructions } from './score-description-grounding.mjs';

export const scoreDescriptionVersion = '2026-09-11.score-descriptions6';
export const publicVerificationWordingInstructions =
  '公开的题目、点评和其他问题不写 AI 浏览器验收、AI 网页验证、人工智能浏览器检查等评测流程标签。点评直接说明真实操作、页面结果及影响，例如只有日志实际证实后才写：勾选附加页后，旧图和画布标记会一起更新，取消后原图片保持不变。确需交代核验来源时用本次页面检查、复查时等客观说法，不改成被测模型已经验证或我人工亲测。未验证的部分仍如实说明未验证，不能删掉缺陷、改变分数或夸大覆盖范围。AI 评分来源保留在来源字段，验收方式及精确证据保留在内部记录；这些记录和原始轨迹不作文字替换。';
export const publicScoreWordingInstructions =
  '五维 descriptions 不出现 AI 身份、工具或模型名称，例如 AI、AI 浏览器、Codex、Claude CLI、Claude Code、GPT、ChatGPT、AI Agent、AI 模型、人工智能或智能体；也不写模型认为、模型完成了、模型先检查等以模型或编程助手为动作主体的说法。直接写真实动作、结果和影响，例如浏览器实测显示、本轮先检查随后修改、点击按钮后页面、执行该命令后返回、复核确认，并接上本轮具体事实，不机械套用句头。编辑、读取、查找和测试操作用普通话说明，不罗列 Bash、Read、Write、Edit、Playwright 等工具名；必要的业务输入、命令参数或实际报错仍须解释清楚。数据模型、业务模型等业务概念可正常使用。交付时的操作、交付说明中的自报和后续复核须分清，不能删掉主体后把复核操作说成原作业已经完成，也不能改写成人工亲测。此规则仅约束五维公开描述，source、Harness、模型配置和内部证据保留真实名称；分数、缺陷、未验证范围、原始输出和原生轨迹保持不变。';
const limit = 4;
...
export function scoreDescriptionStyleIssues(text) {
  ...
  // Match known presentation leaks, including Chinese-adjacent and fullwidth
  // names. This is a bounded review trigger, not automatic text replacement.
  if (
    /(?<![a-z0-9_])(?:ai(?:[\s-]*agent)?|codex|gpt(?:[- .]?\d[\w.-]*)?|chatgpt|claude(?:[\s-]*(?:code|cli))?|openai|anthropic|copilot|cursor|gemini|deepseek|qwen|sonnet|opus|haiku|bash|read|write|edit|glob|grep|playwright|pytest|vitest|jest)(?![a-z0-9_])/iu.test(
      normalized,
    ) ||
    /人工智能|智能体|编程助手|大语言模型|大模型/u.test(normalized) ||
    /(?<!数据|业务|领域|请求|响应|状态|存储|评分|风险|预测|页面)模型(?:的)?(?:在|先|随后|只|仅|未|没有|已经|已|仍|也|还|能|不能|正确|错误|按|对|将|把|用|通过|声称|自报|认为|完成|检查|验证|实现|修改|确认|定位|识别|发现|判断|执行|报告|返回|给出|生成|补充|调整|读取|运行)/u.test(
      normalized,
    )
  )
    issues.push(
      '五维描述不出现 AI 身份、工具或模型名称，也不用模型作为动作主体；按原件写具体操作、结果和影响，区分交付过程与后续复核，保留分数、缺陷、未验证范围及内部真实来源，不自动删词或编造人工经历',
    );
  if (
