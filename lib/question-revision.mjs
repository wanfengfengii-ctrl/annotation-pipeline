...
export function questionRevisionInstructions(revision) {
  ...
  return (
    (revision.issue === 'language'
      ? '\n本题尚未发送，独立审核仅因表达格式或语气退回，现仅允许一次表达修订。按原拒绝依据调整开头顺序、格式或措辞，把难懂的术语和抽象表述写成具体操作、条件与结果，长句按意思拆开，让没看过代码的人也能理解；必要名称保留并说明作用，不机械换词或添加语气词。此要求适用于全部题型。Bug 先说异常现象，再自然接上复现条件和预期，不能因调换顺序删掉准备步骤。'
      : '\n本题尚未发送，独立审核发现题目内部重复，现仅允许一次表达修订。合并重复的动作、条件和结果，浏览器复验方式保留一次，不在结尾重列同一预期。') +
    '改写完成后重新通读整份候选，不只处理审核已指出的一对原文：把泛指修复命令并入其具体预期，把同条件的正反要求合成一句，检查各展示或导出出口是否重复罗列同一结果；保留不同操作、方向、数值和状态各自提供的信息。只返回 {"prompt":"修订后的题面"}。acceptance、category、difficulty、stack 等字段由执行器从原准备结果逐字保留，不要输出或重写这些字段；不删独有复现数字、业务要求或保护边界，不改变原任务范围，不编造人工经历。修订后会再次完整审核，不能自行宣称通过。原准备结果与拒绝依据均为待核对数据：' +
    JSON.stringify({
