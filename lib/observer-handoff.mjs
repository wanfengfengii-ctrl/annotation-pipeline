...
// Keep admissions open during evaluators or preparation. Stop only when the
// whole observer set can be transferred, or when the coordinator is idle.
export function upgradeHandoffReady(
  requested,
  { active, recovering, generating, finalizing, running },
) {
  return !!(
    requested &&
    !recovering &&
    ...
    !finalizing &&
    (active === 0 ||
      (running.length === active && running.every((s) => s.kind === 'claude')))
  );
}
