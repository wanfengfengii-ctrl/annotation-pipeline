...
function maskedPackagePermission(text, call) {
  ...
  // Require an actual package-manager call and both native apt diagnostics;
  // reading a source file or an old error log must not count as a new denial.
  if (
    call?.name !== 'Bash' ||
    !/(?:^|[;&|\n])\s*(?:\/usr\/bin\/)?apt(?:-get)?\s+(?:-[^\s]+\s+)*(?:install|update|upgrade|remove|purge|autoremove)\b/.test(
      call.input?.command || '',
    )
  )
    return null;
  return /^E:\s+Could not open lock file \/var\/(?:lib\/(?:dpkg|apt)|cache\/apt)\/[^\r\n]+\(13: Permission denied\)\s*$/im.test(
    text,
...
export function auditPermissionTraces(files) {
  ...
  return {
    ...
    checksVersion: '2026-09-10.shell-permissions1',
