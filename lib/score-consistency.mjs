export const scoreConsistencyVersion = '2026-09-12.score-consistency6';
...
export function scoreConsistencyInstructions() {
  return `评分一致性规则 ${scoreConsistencyVersion}：每个非满分点评在生成时就自然包含具体位置、实际行为和客观后果。具体位置优先指用户能理解的页面入口、操作或开发步骤，说明那里实际发生了什么、影响哪项使用或开发工作；不要求在 descriptions 写文件路径或日志行号。精确定位保存在 evidenceRefs，内部有证据但公开点评只写泛泛的问题名称也不合格。所有点评都须依据本轮事实，不能把只读检查、模型自报和实际运行混为一谈；不要捏造人工误操作、额外轮次、耗时、百分比或测试失败，未观察到的后果不写成事实。选档比较放在 processFindings，descriptions 用平淡、通俗的一段话解释实际表现，不加评分档位结尾。高分点评不要混入其他维度的问题再用否定句解释为何仍给满分。已复现问题必须保留在所属维度及 artifactFindings 或 processFindings；不能通过删词、同义替换、隐藏缺陷或随意调分迎合平台。确有本维实质不足时按原分档重新评分，不能以属于实现问题为由自动排除指令遵循：须逐条对照原题约束，确认问题是否违反原题。输入校验中的错误提示、已修复的历史失败及其他维度问题应结合证据判断，关键词本身不决定分数。若核对后确认本维为5分，且描述的是正常拦截无效输入，请写清触发条件、实际提示及处理结果，例如缺少分支连线时会定位对应节点，并把该用例排除在覆盖统计之外；不要笼统写规程错误或检查错误。错误、不能、偏差不是必须照抄的业务术语，描述正常校验行为时可以按真实场景重新组织表达。禁止删词或换词规避检查指的是隐藏真实缺陷，不是要求在已确认正常的描述中保留歧义词。确有本维缺陷则仍说明具体后果并据实调整分数，不能为了消除词语而抬分或隐去问题。SOLO 已观察到满分点评中的错误、不能、偏差会触发一致性质检，生成后须核对这些歧义并给出清楚、具体、口语化的本维评价；事实仍有冲突时保留问题，不能制造无问题结论。\n指令遵循的义务以原始轨迹中实际发送的题目及有效上下文为准。内部 acceptance、独立验收计划、评分器提示和交付检查是核验参考，未发送给被测模型的补充要求不能反推为模型漏做的指令。每个“遗漏明确要求”的扣分理由须指出原题相应要求及实际违背；不能仅引用内部清单。原题只要求引用测试并区分已有断言与未覆盖边界时，不额外要求执行测试或声明未运行；最终答复没有声称运行通过时，也不能只因缺少这句声明判为虚假成功。实际误导、说明错误、约束违背和各维 rubric 中的过程表现仍按真实证据评价，不机械提高分数。正确做法中的字段、条件、函数语义及调用顺序须逐项核对当前源码，不能用建议替代真实实现。`;
}
