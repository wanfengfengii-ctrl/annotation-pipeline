...
export function questionAuditInstructions(options = {}) {
  ...
  return `${questionInstructions(options)}\n独立审核候选实际 prompt 的内容与格式，不能只相信作者自述。原始目标提供业务范围和事实依据，题目表达、字数和内部去重只审查将要发送的候选 prompt；不能因原目标中的重复已被候选合并而继续拒绝。questionCompliant 仅在全部适用标准通过时为 true；questionChecks 按下列 ID 顺序逐项返回 ID：正文依据或失败原因，缺失明确写缺少。workflowFeatures 列出正文中 4 至 6 项相互关联的功能或已有业务操作，businessDetails 列出 1 至 2 个正文中的具体业务细节；数量不足或超出时如实返回，禁止编造凑数。reason 说明不合格项，questionCompliant=false 时 allowed 必须为 false。\n${Object.entries(
    applicable,
  )
