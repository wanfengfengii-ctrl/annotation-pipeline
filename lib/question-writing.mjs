  ...
  return (
    ...
    (isBugRepair(options)
      ...
      : options.category && !questionHasTitle(options)
        ...
        : '') +
    (legacyRepair
      ? '\n本轮范围继承说明：执行器已核验，本轮是网页规则启用前已实际执行的历史题目在原会话中的真实 Bug 修复。沿用原题面向的用户、既有业务操作和界面形态；web 和 layout 按原题范围核验，原题无网页时写明不适用，不能强行新增网页、虚构浏览器操作或改变原题验收范围。其余格式、语气、业务范围、禁出、雷同、难度及真实复现审核全部保留，不以段落、操作或细节的数量判定合格。此范围继承仅适用于本次关联修复，不适用于首题、0-1、Feature、理解或重构独立新题。'
      : '')
  );
}
...
export function questionFormatIssues(text, options = {}) {
  ...
  const { title, paragraphs, body } = questionParts(text, options);
  const issues = [];
  if (
    hasTitle &&
    (!title ||
  ...
  if (!body.trim()) issues.push('题目需有实际需求正文');
  if (
    !hasTitle &&
    paragraphs.length > 1 &&
  ...
  const applicable = isBugRepair(options)
    ...
    : questionHasTitle(options)
      ...
      : {
          ...criteria,
          language:
            '本题没有项目名称、标题或编号，直接写可执行的需求正文，按内容自然分段。' +
            criteria.language,
        };
  return `${questionInstructions(options)}\nwordingDuplicatePairs 只列实际重复句对；没有重复时必须返回空数组 []，不得填写无重复、未发现重复等说明，这些说明放在 redundancy 检查中。不能用空数组掩盖实际重复。\n独立审核候选实际 prompt 的内容与格式，不能只相信作者自述。原始目标提供业务范围和事实依据，题目表达和内部去重只审查将要发送的候选 prompt；不能因原目标中的重复已被候选合并而继续拒绝。questionCompliant 仅在全部适用标准通过时为 true；questionChecks 按下列 ID 顺序逐项返回 ID：通过，正文依据 或 ID：不通过，失败原因，缺失明确写不通过及缺少的依据。表达难懂归入 language，不因必要业务术语本身否定功能或难度；其他项也须明确核对并给出通过或不通过，便于仅表达失败时按原范围修订。workflowFeatures 如实列出正文中的关联功能或已有业务操作，businessDetails 如实列出正文中的具体业务细节；没有相应内容时返回空数组，不编造凑数。两者不设数量门槛，按业务范围、事实和需求是否清楚完整判断，不能仅因字数、段落数、操作数或细节数判定不合格。reason 说明不合格项，questionCompliant=false 时 allowed 必须为 false。\n${Object.entries(
    applicable,
  )
...
export function assertQuestionAudit(value) {
  ...
  for (const field of ['workflowFeatures', 'businessDetails']) {
    ...
    if (
      ...
      throw Error(`题目内容审核 ${field} 需为真实且不重复的正文依据数组`);
  }
}
