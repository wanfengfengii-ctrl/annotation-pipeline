...
const scoreExplanationInstructions = [
  ...
  '后续独立验收未完成，不等于原作业漏做或原题功能不可用。临时脚本的文本匹配、空白处理、定位或断言假设错误，以及未记录实际值的断言失败，不能单独作为产品缺陷、指令违背、推理遗漏或虚假成功的扣分依据，也不能因后续检查中断就把各维分数上限设为4。先核对断言具体比较了什么、页面或命令实际返回什么、是否确实违背原题，并与原始轨迹的真实测试输出分别归因。后续没有执行的步骤照实记在 artifactFindings，不否定原作业已有的通过证据，也不把原测试通过扩大成所有功能通过。缺少实际观察时保留证据缺口并补验，不编造缺陷、不预设满分；已证实的产品问题、原作业的操作失误及各维不足仍按 rubric 独立评价。交付校验提出的建议分数只是复核线索，不直接照抄。',
