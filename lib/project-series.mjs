...
export function seriesPrompt(task) {
  return task.projectSeries
    ? `项目连续出题规则：同一个项目、同一个 Claude 会话、最多 10 次交互（失败调用也占次数）。项目目录 ${task.projectSeries.directory}，原仓库只作为快照载体；不修改目录外的现有业务。首题必须从零构建独立项目，题型为 0-1 代码生成。截断后继续原题时沿用原题分类和验收目标，0-1 续写仍为 0-1，不等于再建项目。后续基于该目录的真实产物出题，禁止再次从零另建项目、换皮重复或拆小题凑轮次。已发现的真实缺陷优先 Bug 修复；项目基础可用后选择 Feature 迭代，其次代码理解 ≈ 代码重构，再考虑工程化或测试。不要为凑比例凭空假设 Bug。所有题仍须通过禁出、雷同和难度审核。`
    : '';
}
