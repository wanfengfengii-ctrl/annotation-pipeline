...
import { isGatewayContinuation } from './gateway-continuation.mjs';
import {
...
import { wasSent, projectQuotaComplete } from './project-recovery.mjs';
...
export function repairBatchInstructions() {
  return `Bug 修复按业务关联确定范围：一题围绕同一业务流程中有真实复现证据的问题，说明受影响的已有操作、发生条件和预期，不限制操作或问题细节的数量。关联问题可以在同题处理；涉及独立流程或超出中小型范围时按真实边界分批，不按固定数量拆分，不为凑数增加问题，也不合并不同条件来掩盖范围。repairCheckIds 只填本题实际覆盖的复现检查 ID，未选检查和完整日志仍留在原验收报告中，可在 reason 或 projectEvidence 说明后续范围，不写入当前题面或 acceptance。prepare 仅展开本题已选范围，不从前序报告补回未选缺陷。题面与验收必须逐项沿用真实复现记录里的控件、操作和先后顺序，不能把点击时间点按钮改写成拖动进度条，或把点击、输入、拖放互换；证据只覆盖哪种操作，就只描述哪种操作，缺少依据的交互不能补写成已发生的事实。后续 project-next 和 next 必须逐项核对前序已复现但本题未覆盖的问题，在 reason 或 projectEvidence 保留未解决清单和原报告、检查 ID；本轮局部修复 passed 不能证明这些问题已解决，不能据此写 complete 或 baseComplete=true。缺少剩余问题的新回归证据时明确标记未复核，先补独立复验，不丢弃旧证据或猜测已修好。完成本批后重新验收，只有新报告仍复现的问题才能进入下一批；同会话累计最多 ${sessionLimits.maxBugRepairs} 轮 Bug 修复，已用满仍有问题时 needs_input，不新增第三轮、不换会话绕过限制，也不把尚未修复的缺陷标为完成。每批都重新通过原有禁出、难度及题面审核，按实际需求检查完整性，不以操作或细节数量作为通过条件。`;
}
    ...
    freshCategories.map((c) => [
      ...
      task.turns.filter(
        (r) =>
          ...
          r.category === c &&
          (wasSent(r) ||
            (!r.projectRetry && ['queued', 'running'].includes(r.status))),
      ).length,
    ]),
  );
}
...
export function canRepair(task, turn) {
  ...
  if (
    ...
    return false;
  const turns = sessionTurns(task, turn).filter(
    (r) => !isGatewayContinuation(r),
  );
  return (
    ...
    !sessionTurns(task, turn).some(
      (r) => r.permissionAudit && !r.permissionAudit.passed,
    )
  );
}
export function shouldFinishSession(task) {
  ...
  return !!(
    ...
    turn.sessionFinished ||
    claudeCallCount(task, questionRoot(task, turn)) >= sessionLimits.maxCalls ||
    sessionTurns(task, turn).filter((r) => !isGatewayContinuation(r)).length >=
...
export function projectDecision(task, turn, config) {
  ...
  if (['complete', 'needs_input'].includes(d.action))
    return {
      notice: projectQuotaComplete(task)
        ? d.reason
        : d.reason + '；仅结束本会话，项目题额未满，保留当前项目继续规划',
      finishSession: true,
    };
