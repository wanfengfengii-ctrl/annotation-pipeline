  ...
  for (const incident of Object.values(state.incidents)) {
    ...
    const unchangedAttempt = state.jobs.findLast(
      (j) =>
        ...
        j.conditionsKey === conditionsKey &&
        ['failed', 'needs_input', 'uncertain'].includes(j.state),
    );
    ...
    if (delivered || continued || systemRecovered) {
      ...
      incident.state = 'waiting_conditions';
      incident.result = '沿用已有诊断和复核意见，等待源码版本或真实证据变化';
      incident.previousJobId = unchangedAttempt.id;
    } else if (incident.state === 'waiting_conditions') {
      incident.state =
        incident.attempts < config.maxAttempts ? 'ready' : 'needs_input';
    } else if (
      currentIssue &&
  ...
  for (const i of Object.values(state.incidents).sort(
    ...
    if (
      state.jobs.some(
        (j) =>
          j.incidentId === i.id &&
          j.conditionsKey === selfHealConditions(i, snapshot) &&
          ['failed', 'needs_input', 'uncertain'].includes(j.state),
      )
    )
      continue;
