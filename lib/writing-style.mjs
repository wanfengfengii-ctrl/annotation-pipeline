...
const fields = {
  ...
  // Internal findings retain exact evidence and may use paragraphs. Only public
  // feedback needs prose rewriting; rewriting must not alter those findings.
  score: ['descriptions', 'other'],
};

export function writingInstructions(stage, questionContext = {}) {
  if (!fields[stage]) return '';
  const common = `表达要求只应用于 ${fields[stage].join('、')} 字段：\n${workflow.writingStyle.join('\n')}\n${publicVerificationWordingInstructions}${stage === 'score' ? '\n' + publicScoreWordingInstructions : ''}`;
...
export function proseIssues(text, { paragraphs = false } = {}) {
  ...
  const issues = [];
  if (
    /(?:\bAI\b|人工智能)(?:[\s·-]*(?:驱动|自动化?|独立|的))*[\s·-]*(?:浏览器|网页|页面)[\s·-]*(?:验收|验证|检查)/iu.test(
      text.normalize('NFKC'),
    )
  )
    issues.push(
      '公开文字不使用 AI 浏览器验收等评测标签，按真实证据说明页面操作、结果和影响；保留核验归因、评分来源和内部原件，不伪装人工经历或机械删改事实',
    );
  if (!paragraphs && /\r|\n/.test(text))
    ...
    const normalized = texts.map((text, i) => {
      ...
      const found = noQuestion
        ...
        : question
          ...
          : proseIssues(s);
      if (stage === 'score' && field === 'descriptions')
        found.push(...scoreDescriptionStyleIssues(s));
