...
export function sessionFinalization(task) {
  ...
  if (last.status === 'failed') {
    if (unsentFailure(last)) {
      // A failed draft in an existing conversation can still be revised and
      // submitted there. Closing it here makes the normal retry impossible.
      if (
        last.repairOf ||
        last.continuationOf ||
        frozenPreparationFailure(last)
      )
        return null;
      return { ...identity, reason: 'unsent-candidate-rejected' };
    }
    if (
