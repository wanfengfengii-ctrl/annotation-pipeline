...
export function policySessionContext(task, turn) {
  ...
  const isRepair = (item) =>
    !isGatewayContinuation(item) &&
    (!!item.repairOf || item.category === 'Bug 修复');
  ...
  const currentLogicalTurn =
    priorSameSession.filter((r) => !isGatewayContinuation(r)).length +
    (isGatewayContinuation(current) ? 0 : 1);
  const currentBugRepairOrdinal = isRepair(current)
    ? priorSameSession.filter(isRepair).length + 1
    : 0;
  const recordedLogicalTurns = sameSession.filter(
  ...
  return {
    ...
    countingRule:
      '同一项目的不同 questionRootId 属于独立会话，不把其他原题的 Bug 修复计入当前会话；未发送的业务草稿计一题，受控504继续不计新题或新Bug，实际发送后均计调用。排除或失败记录仍保留额度。同会话最多初始题加两道Bug，所有实际调用最多十次；本上下文不替代其他执行校验，也不授权重发已发送或状态不明的题目。',
  };
}
