...
2026-09-11 16:00 批次的原生 ZIP 在真实表单被拒绝，提示轨迹文件不是 .jsonl 格式。只读核对上传控件为 `accept=".jsonl"`、`multiple=false`；此前已接收的 ZIP 回执仍按实际结果保留。当前使用 `2026-09-11.native-verbatim-jsonl2`：`solo-native-attachment.mjs` 先核验完整原生 projects 导出，只有清单恰好包含一个 JSONL 文件时，才直接复制其原始 Buffer 作为附件，保留原文件名。正文、空白、BOM、换行和编码字节全部不变，不脱敏、不重序列化、不裁剪，也不将 ZIP 改后缀伪装成 JSONL。

完整导出包含子会话、其他 JSONL 或 JSON 文件时暂缓上传，不能只取主会话，不能拼接多份原件。所有原生文件及目录仍完整保存在本地归档中；空目录记录在本地核验清单，不会混入 JSONL 正文。评分、运行验收、源码、内部清单和 provenance 不随轨迹附件上传。用户要求的是原始内容不改动；此前文档中一律使用完整 ZIP 的规定是旧平台适配方式，现由这一单文件完整性条件替代。

权限检查还核对 Bash 管道中被成功退出码掩盖的原生错误。实际执行 apt 时，即使使用 `-o` 指定临时目录，缓存清理仍可能返回 Permission denied；后面的 `tail` 成功不能将这次拒绝消除。2026-09-10 晚间复核发现此类漏检，同一完整原生会话的三条记录已按固定 ID 暂缓上传。原始轨迹和已有点评保留，不以重新打包消除拒绝。用户随后明确要求原样上传这三条，因此仅对这三个固定记录建立本机 permission-admissions.json 授权。例外绑定任务、轮次、SessionID、PromptID、原件清单、实际附件摘要、检查版本和确切错误集合；任一项变化仍阻止上传。审计继续记录 permissionPassed=false 和真实 denialCount，另存用户授权，不伪造通过。其他禁止上传记录不受影响，授权清单不提交 Git。旧授权若绑定 ZIP 摘要，不能自动转移到 JSONL 附件；历史后台导出授权的附件绑定同样保留。

准备时逐文件核对完整原件清单的 SHA-256 和字节数，写出 JSONL 后再逐字节比对；只读解析仅用于原生标识和权限检查。附件外的本地审计记录 `byteIdentical: true`、`redactions: 0`、`completeNativeFileSet: true` 和相同的原件/附件摘要。文件缺失、权限拒绝、标识不匹配、内容变化均阻止生成，不能删改内容使其通过。平台 20 MB 限制应用于实际发送的未压缩 JSONL，超限不压缩绕过、不裁剪轨迹。

发送前从原件重新构建并校验，并核对浏览器选中的实际文件路径、名称、大小、摘要和字节内容；旧 ZIP 或替换版 packet 即使摘要自洽也不能继续发送，须在允许的新批次重新 prepare。格式修复不修改正在执行或已收尾批次的固定成员、附件摘要和台账；已发送或结果不明的记录只查回执，不因格式改变重传。已上传附件不会因本地修复而自动改变；需单独核对平台是否允许返修，并保留旧版附件及真实远端回执，不改写历史为原样上传。
...
每条数据必须通过现有题目、权限与交付校验，以及最终原终端导出和容器清理检查；旧后台导出不冒充新协议最终原终端导出。外发附件必须满足上述完整导出仅有一个原生 JSONL 的条件，发送前重新验证原样字节、文件清单、内容摘要和 20 MB 限制。原始轨迹与上传文件内容必须一致。任何附件、收尾证据或绑定异常都进入 blocked，继续处理其他可提交记录。

2026-09-10 用户要求减少本地上传拦截，由平台执行质量检查后按实际退回结果返修。内部评分日志、项目数据库等不随原生附件发送，它们的脱敏或内容扫描结果保留为本地提示，不再阻止 SOLO 上传。`verifySubmissionPackage` 的 `native-only` 用途仍核对原件、提交副本清单、文件摘要和原终端最终回执，但不以内部内容扫描结论作为上传门槛；默认内部副本核验继续保持严格检查，不能把带提示的内部 ZIP 当成已脱敏附件外发。原生附件仍由独立附件生成器从完整原件生成，上传前核对实际 SessionID/PromptID、完整目录和原样字节。禁止上传标记、重复提交保护和原有固定记录授权继续有效。不要将本地准备成功写成平台质检通过。
...
1. 独立上传任务每天北京时间 08:00、10:00、12:00、14:00、16:00、18:00、20:00、22:00 运行，生产监测与故障恢复由后台自愈服务承担，不再每 15 分钟唤醒模型巡检。上传时先执行 `node scripts/solo-schedule.mjs --due` 并当场确认登录，不另行安排半点登录预检。uploadPaused=true 时不创建或恢复上传批次；已有 active 不接管；仅当前上传者完成已点击提交的回执核对后，或确认原上传者结束并按既有租期规则核对后，才能按原 slot、attemptId 以 waiting_window 收尾。08:00 后优先恢复 waiting_window 和因登录暂停的原批次，保持固定成员。新批次先运行 `--prepare`，将完整输出保存为私有 plan.json，再执行 `solo-schedule.mjs --claim PLAN_JSON`，即使 canClaim=false 也要执行这次 claim，固定本批成员及原件摘要。无新数据会直接结束空批次。登录未就绪时保存 waiting_login；不能登录失败就丢掉本次批次。resume 模式执行 `--claim`，不传新成员。仅 claimed=true 才进入上传，并保存返回的 slot、attemptId。没有新数据且无新的异常时保持安静。
...
后续出题会单独保存规划结果，原评分与归档保留在 `pre-replan-result.json`。最终提交队列从该原件恢复回填；旧队列的 `.done` 仅表示曾完成扫描，不能替代逐条提交回执的 `.delivered` 确认。补回填仍校验原题、容器、归档和原终端回执，不改题目、评分、轮次或轨迹，也不直接向质检平台创建记录。暂缓提示显示具体交付问题；首轮被用户明确禁传时，后续轮次显示首轮禁传的关联原因。
