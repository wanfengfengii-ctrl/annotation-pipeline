...
2026-09-10 用户确认排除 Webhook 最早三条（确认时当前列表 14—16）。具体记录与当时列表固定在 `.runner/solo-upload/batches/user-selected-20260910/selection.json`。被排除首轮的同会话后续轮次不能改号补空缺。
...
SOLO 附件由 `solo-native-attachment.mjs` 从已核验的完整原生 projects 目录构建，保留子会话、文件及目录；不混入评分、运行验收、项目源码、内部清单或 provenance JSON。脱敏映射和原件摘要保存在 ZIP 外的本地审计文件。原生文件原件不改写，目录或文件缺失、权限拒绝、标识不匹配、脱敏失败均阻止生成。平台 20 MB 限制应用于实际发送的原生 ZIP。
...
每条数据必须通过现有题目、权限与交付校验，以及最终原终端导出和容器清理检查；旧后台导出不冒充新协议最终原终端导出。内部提交副本和源归档先核验，外发附件仅包含完整原生轨迹目录，发送前重新验证文件清单、内容摘要、20 MB 限制及当前已配置密钥的脱敏结果。原始轨迹不改写。任何附件、收尾证据、敏感内容或绑定异常都进入 blocked，继续处理其他可提交记录。
