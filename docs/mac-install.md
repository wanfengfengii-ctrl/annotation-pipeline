...
安装脚本检查依赖和登录，生成仅当前用户可读写的 `.dev.vars`，应用所有数据库迁移，构建 `annotation-pipeline/claude-code:2.1.266-20260910`，最后构建页面。Dockerfile 固定基础摘要、Node 22.22.1 和 Claude 2.1.266，保留现有镜像的入口、模型、网关和上下文；构建不会发送模型题目。
