...
待执行队列为空且存在空闲执行槽时，Codex 只读检查仓库并生成一个独立任务，自动入队。出题同样沿用 Codex CLI 模型配置并保存轨迹；模型收到最近 200 个同仓库任务标题以避免语义重复，服务端用规范化 Prompt 指纹做精确去重。语义去重属于模型判断，并非绝对保证。并发补充由数据库原子条件限制，网络失败保留待回写的出题结果，不重新调用模型。
...
```sh
node --experimental-strip-types --test tests/rules.test.mjs tests/codex-stages.test.mjs tests/scheduler.test.mjs
npx tsc --noEmit
npm run build
```
