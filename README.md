...
每个新项目只创建一个新容器，绑定全新且完全为空的 `.runner/<task-id>/workspace` 到 `/workspace`。不复制仓库、隐藏文件或 CLI 配置；Claude 可在已开始的任务内按需求获取项目材料。同一项目的后续题保持同一个交互进程，每项目最多 10 次。容器不使用 `--rm`，不 start/restart、不使用 continue/resume 参数。
...
每个项目只有一个由容器内 Claude 生成的真实 SessionID；后续 Prompt 写入同一个存活的终端进程。每次发送前由服务端原子预占调用次数，失败/不确定调用仍占次数；阶段缓存恢复不重复调用、不重复计数。10 条题目或累计 10 次 Claude 调用任一达到上限即停止新增；允许只重做评分和归档。旧项目不强制迁移为 0-1，新的硬上限仍适用。模型继续沿用两个 CLI 的现有配置。
...
依据 [Mac 作业要求](https://docs.qq.com/document/DVVdadUV2dWV4UXZX)，实现见 [迁移说明](docs/docker-workflow-2026-09-09.md)。每轮完整复制容器 `/home/node/.claude/projects`，保留目录结构并比较 SHA-256。项目完成、达到上限或手动结束后，用两次 Ctrl-D 退出，再次导出并核验完整轨迹和代码目录；仅核验通过才删除该项目容器。导出失败保留容器并显示原因，下次调度重试。没有使用容器清理通配符。
