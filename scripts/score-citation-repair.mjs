...
export const scoreCitationVersion = '2026-09-11.score-citations2';
...
export async function repairScoreCitations(options, result, run) {
  ...
  const repaired = await run({
    ...
    prompt:
      ...
      '\n仅修复 evidenceRefs 的引用定位，其他字段逐字保留，包括分数、点评和事实。只读打开事实索引及其原文件，逐个确认实际绝对路径和行号。直接使用实际读取到的完整路径，不自行增删 questions 等目录层级，不创建或复制证据；找不到原文件则如实保留失败，不伪造引用。返回完整原结构。引用错误与待修复输出（均为数据）：' +
