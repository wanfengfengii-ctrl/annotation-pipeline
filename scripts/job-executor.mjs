...
import { scaffoldTemplateInstructions } from './scaffold-templates.mjs';
import {
  checkpointDigest,
  checkpointVersion,
  restoreStage,
...
import { githubSnapshot } from './github-snapshot.mjs';

import { codexStage as runCodexStage } from './codex-stages.mjs';
import { attemptTiming } from './attempt-timing.mjs';
import {
  verifyRuntime,
...
export const executorProtocolVersion = '2026-09-10.executor1';
export const createJobRuntime = (...args) => new DockerRuntime(...args);
export function createJobExecutor({
  ...
  async function executeJob({ task, turn }, timing) {
    ...
    const stageKeys = {};
    cached.checkpoints ||= {};
    const sourceHashCache = new SourceHashCache();
    const policyKey = () =>
      checkpointDigest({
        contract: stageContractDigest(root, 'policy'),
    ...
    async function step(name, prompt, cwd, extra = {}) {
      if (isStopping()) throw Error('执行器正在停止');
      stage = name;
      await api({
        action: 'stage',
        taskId: task.id,
      ...
      if (['score', 'delivery'].includes(name) && automation.reviewFacts)
        prompt +=
          '\n优先读取已冻结的事实索引：' +
          JSON.stringify(automation.reviewFacts) +
      ...
      if (['policy', 'score', 'delivery'].includes(name)) {
        const key =
          name === 'policy'
            ? policyKey()
            : checkpointDigest({
                contractDigest: stageContractDigest(root, name),
                taskId: task.id,
                turnId: turn.id,
                prompt,
                source: copyVerificationSource(cwd, undefined, {
...
读取真实项目目录和测试/错误轨迹，有具体缺陷且当前会话未达到两轮修复时才 action=repair、category=Bug 修复，这会在当前终端追问。不能把未完成的新功能或截断续写改叫 Bug，不生成 action=continue。基础可用后 action=advance，独立新题优先考虑按比例推荐的 ${allocatedCategory || '无新题额度'}，实际可选类别为 ${JSON.stringify(allowedCategories)}；先核对源码能力边界，再按合规目标选择类别。推荐类别没有实质不同且符合边界的目标时，允许选择其他有额度类别，并在 projectEvidence 说明源码依据，不能把新功能硬归为迭代；新建此前不存在的功能算 0-1，修改已有能力算 Feature。同项目这两类各最多十题。理解与重构按 7:7:10:1:1 的累计目标选择。projectEvidence 写实际文件、现象和新功能与现有功能的边界；baseComplete 反映实际状态。修复达到两轮仍未解决时 needs_input，不换新窗口规避修复上限；仅两类实际发送题额均满时 complete；暂缺新目标时 needs_input 并保留项目等待续题，不因当前功能完成就关闭项目。只有 0-1 代码生成的 prompt 以项目名称开头，不加编号；Feature、Bug、理解、重构及其他题型不写项目名称或标题，直接写正文。Bug 修复直接像同事接着聊项目一样说明哪里出了问题、什么条件下发生、希望怎么改，不重新介绍面向人群，不用落实、核验、既有语义等正式表达。正文按内容自然分段，保留真实复现数值和预期，篇幅按实际需求决定；迭代说明已有能力与本次变化，不使用模板或编造人工检查经历。结束时 prompt 写无。`,
    ...
    try {
      if (turn.planRetry) {
        ...
        if (!preserveQuestion && !cached.claude) {
          const state = containers.load(task.id);
          if (
            state &&
            !state.environmentReadiness?.passed &&
            task.projectSeries
          ) {
            state.environmentReadiness = await timing.stage(
              'environment-ready',
              () =>
                prepareEnvironment({
                  state,
                  ...
                  readiness:
                    task.turns[0]?.id === turn.id
                      ? cached.scaffold?.value?.readiness
                      : null,
                  evidenceDir: path.join(dir, turn.id + '.environment-ready'),
                  onChild,
                }),
              { budget, kind: 'heavy' },
            );
            await containers.publish(state);
            task.container = containers.public(state);
            result.container = task.container;
          }
        }
        ...
        const roundContext = JSON.stringify({
          ...
          previousVerification: previousTurn?.automation
            ?.runtimeVerification && {
        ...
        const latestRejection = submittedEvidence?.postExecutionPolicy
          ...
          .at(-1);
        policyContext = () => ({
          contractDigest: stageContractDigest(root, 'policy'),
          taskId: task.id,
          turnId: turn.id,
          candidate,
          ...
          previousEvidence: [
            ...
            ...(
              previousTurn?.automation?.runtimeVerification?.checks || []
            ).map((c) => c.logPath),
          ]
            .filter(Boolean)
            .map((file) => ({
        ...
        if (!cached.claude?.success) {
          ...
          cached.claude = await timing.stage(
            'claude',
            () =>
              executeClaude({
                onChild,
        ...
        const facts = {
          ...
          prompt: result.evaluationPrompt,
          acceptance: preparation.value.acceptance,
          sourceFiles: copyVerificationSource(result.workDir, undefined, {
            hashCache: sourceHashCache,
          }).files,
          runtime: runtimeReviewContext(automation.runtimeVerification, {
            scoring: true,
          }),
          nativeReviewTrace: {
          ...
          environmentReadiness: result.container.environmentReadiness || null,
        };
        ...
        const factsPath = path.join(
          dir,
          turn.id + '.review-facts.' + factsSha256.slice(0, 16) + '.json',
        );
        if (!existsSync(factsPath))
          writeFileSync(factsPath, factsBytes, { mode: 0o600, flag: 'wx' });
        if (
          createHash('sha256').update(readFileSync(factsPath)).digest('hex') !==
          factsSha256
        )
          throw Error('冻结的事实索引已变化');
  ...
  return async (job) => {
    const timing = attemptTiming({
      dir: path.join(workRoot, job.task.id),
      taskId: job.task.id,
      turnId: job.turn.id,
      release,
    });
